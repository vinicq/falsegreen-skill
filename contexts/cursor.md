...
- C2: test body has no assert at all (HIGH) — exemption: @skip, @xfail, @given
...
Does the test depend on private methods, internal state, or implementation details
that could change without breaking the public contract?
...
2. Mocking a network/disk/time dependency is NOT case 10. Case 10 applies only
   when the mock replaces the unit under test.
3. Classify before judging (Step 3). A characterization test is not a bug.
